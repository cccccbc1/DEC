...
#from lib.dpc import density_peak_clustering
#from lib.knn_dpc import knn_dpc
...
class DEC(nn.Module):
    ...
    def entropy_weight(self, matrix):
        ...
        # q = 1.0 / (1.0 + torch.sum((((z.unsqueeze(1) - self.mu) ** 2) * weight.unsqueeze(0).unsqueeze(0)), dim=2) / self.alpha)

    def forward(self, x):
        h = self.encoder(x)
        z = self._enc_mu(h)
        # compute q -> NxK
        q = 1.0 / (1.0 + torch.sum(((z.unsqueeze(1) - self.mu) ** 2) * self.FeatureWeight.unsqueeze(0), dim=2) / self.alpha)
    ...
    def fit(self, X, y=None, lr=0.001, batch_size=256, num_epochs=10, update_interval=1, tol=1e-10):
        ''' X: tensor data '''
        use_cuda = torch.cuda.is_available()
        if use_cuda:
            self.cuda()
